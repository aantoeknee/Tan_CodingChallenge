...
class DetailsController: UIViewController {
  ...
  // MARK: - Properties
  let IS_FAVORITE = "heart.fill"
  let IS_NOT_FAVORITE = "heart"
  ...
  private func initNavigationBar() {
    ...
    let barButtonImage = UIImage(systemName: isFavorite ? IS_FAVORITE : IS_NOT_FAVORITE)
    barButtonItem = UIBarButtonItem(image: barButtonImage,
                                    style: .plain,
                                    target: self,
                                    action: #selector(addToFavorites))
    barButtonItem.tintColor = isFavorite ? .red : .blue
  ...
  // MARK: - Add to Favorite button clicked
  @objc func addToFavorites() {
    viewModel?.addToFavorites { [weak self] in
